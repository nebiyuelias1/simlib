...
#define LIST_SYSTEM          3  /* NEW: List number for for system . */
#define SAMPST_DELAYS        1  /* sampst variable for delays in queue. */
#define SAMPST_SYSTEM_DELAYS 2 /* NEW: sampst variable for delays in system. */
...
int num_custs_delayed, num_delays_required,
        num_custs_entered, num_custs_required; /* NEW */
...
main()  /* Main function. */
{
    ...
    while (num_custs_entered < num_custs_required) {
...
void arrive(void)  /* Arrival event function. */
{
    ...
    /* NEW: New customer arrived */
    num_custs_entered++;

    transfer[1] = sim_time;
    list_file(LAST, LIST_SYSTEM);
}
...
void depart(void)  /* Departure event function. */
{
    ...
    /* NEW: Customer is departing, so record this. */
    list_remove(FIRST, LIST_SYSTEM);
    sampst(sim_time - transfer[1], SAMPST_SYSTEM_DELAYS);
}
...
void report(void)  /* Report generator function. */
{
    ...
    fprintf(outfile, "\nDelays in system, in minutes:\n");
    out_sampst(outfile, SAMPST_SYSTEM_DELAYS, SAMPST_SYSTEM_DELAYS);
    fprintf(outfile, "\nQueue length (1), server utilization (2) and system length (3):\n");
    out_filest(outfile, LIST_QUEUE, LIST_SYSTEM);
